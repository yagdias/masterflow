...
# # Alignment step
    host_index = scripts.host.build_host_index(reference, threads, build_index)
    ...
    for sample in sample_list:
        PipelineFunctions.bowtie2_build_cobra(f'{outdir}/{sample.id}.megahit', threads, sample.id, outdir)
        PipelineFunctions.bowtie2_align_cobra(sample.r1, sample.r2, threads, sample.id, outdir)
        PipelineFunctions.coverm_cobra(sample.r1, sample.r2, sample.id, outdir, threads)